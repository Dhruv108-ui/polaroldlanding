...
function Blog() {
    ...
    useEffect(() => {
        async function fetchBlogs() {
            ...
            let blogList = blogSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

            // Convert createdAt string to Date object for sorting
            blogList.forEach(blog => {
                const [day, month, year] = blog.createdAt.split('/');
                blog.createdAtDate = new Date(year, month - 1, day);
            });

            // Sort the blog list by createdAtDate in descending order
            blogList.sort((a, b) => b.createdAtDate - a.createdAtDate);
